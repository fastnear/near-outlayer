...
fn normalize_github_url(url: &str) -> Result<String> {
    ...
    if !url.starts_with("https://github.com/") {
        return Err(anyhow!(
            "Only GitHub repositories are supported (got: {url})"
        ));
    }
...
fn validate_build_path(path: &str) -> Result<String> {
    ...
    if path.contains("..") {
        return Err(anyhow!(
            "Build path cannot contain '..' (path traversal attempt: {path})"
        ));
    }
...
mod tests {
    ...
    fn test_validate_build_path_hidden_files_blocked() {
        ...
        for path in hidden_paths {
            ...
            assert!(
                result.is_err(),
                "Hidden file/directory should be rejected: {path}"
            );
        }
    ...
    fn test_validate_build_path_url_encoded_traversal() {
        ...
        for path in encoded_paths {
            ...
            assert!(
                result.is_err(),
                "URL-encoded traversal should be blocked after decoding: {decoded}"
            );
        }
    ...
    fn test_validate_build_path_double_encoded() {
        ...
        assert!(
            result.is_err(),
            "Double-encoded traversal should be blocked: {second_decode}"
        );
    ...
    fn test_normalize_github_url_cache_bypass_prevention() -> Result<()> {
        ...
        for variation in variations {
            ...
            assert_eq!(
                result, canonical,
                "Variation '{variation}' should normalize to '{canonical}'"
            );
        }
