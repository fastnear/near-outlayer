...
export interface EnclavePolicy {
  timeMs: number;       // wall-clock budget
  memoryBytes: number;  // max memory for runtime
}

export interface Invocation {
  source: string;       // JS contract source
  func: string;         // function name to call (globalThis[func] or default[func])
  args: Json[];         // positional arguments
  priorState: Json;     // JSON-serializable state object
...
export interface InvocationResult {
  ...
  diagnostics: { logs: string[]; timeMs: number; interrupted: boolean };
}

export class QuickJSEnclave {
  private mod: QuickJSWASMModule;
  ...
  private constructor(mod: QuickJSWASMModule, rt: QuickJSRuntime) {
    this.mod = mod;
    this.runtime = rt;
  }
  ...
  async invoke(inv: Invocation): Promise<InvocationResult> {
    ...
    try {
      ...
      // Probe: verify deterministic prelude is installed correctly
      {
        ...
        if (probeRes.error) { probeRes.error.dispose(); throw new Error("Prelude probe failed"); }
        ...
        if (probeJson.now !== 0) throw new Error("Deterministic Date.now not installed");
        if (probeJson.hasTimers) throw new Error("Timers not disabled");
      }
...
function jsJSONLiteral(v: Json): string {
  // Serialize JSON into a JS literal
  return JSON.stringify(v ?? null);
}

function buildDeterministicPrelude(seed: string): string {
...
function buildUserCodeEnvelope(inv: Invocation): string {
...
function buildDeterministicEnvelope(inv: Invocation): string {
