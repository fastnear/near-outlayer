...
export default function createQuickJSBrowserMode(
  opts: QuickJSBrowserModeOptions = {}
): SimulatorMode {
  ...
  let enclave: QuickJSEnclave | null = null;
  ...
  return {
    ...
    async execute(input: ExecutionInput): Promise<ExecutionOutput> {
      ...
      const timeMs =
        input.policy?.timeMs ?? defaultTimeMs;
      const memBytes =
        input.policy?.memoryBytes ?? memoryBytes;
