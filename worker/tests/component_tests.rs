...
async fn test_executor_with_minimal_wasm() {
    ...
    let result = executor.execute(&wasm, None, &input, &limits, None, None, &response_format, None).await;
...
async fn test_executor_with_invalid_wasm() {
    ...
    let result = executor.execute(&invalid_wasm, None, &input, &limits, None, None, &response_format, None).await;
